    ...
    return Scaffold(
      ...
      body: NestedScrollView(
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  // ── Avatar row — fixed, avatar always visible ──────────
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.end,
                    children: [
                      // Avatar lifted 30px above banner bottom
                      Transform.translate(
                        ...
                        GestureDetector(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  color: GacomColors.deepOrange, width: 1.2),
                            ),
                            child: const Icon(
                  ...
                  // Compensate for transform offset
                    ...
                    if (verified)
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                              color:
                                  GacomColors.deepOrange.withOpacity(0.4)),
                        ),
                        child: const Row(
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      _StatDiv(),
                      _Stat(_fmtCount(p['followers_count'] ?? 0),
                          'Followers'),
                      _StatDiv(),
                      _Stat(_fmtCount(p['following_count'] ?? 0),
                          'Following'),
                      _StatDiv(),
        ...
        body: TabBarView(
          controller: _tab,
          children: [
            _Grid(posts: _posts.where((p) => p['post_type'] != 'clip').toList()),
            _Grid(posts: _posts.where((p) => p['post_type'] == 'clip').toList()),
            _StatsTab(profile: p),
          ],
        ),
        ...
        return ClipRRect(
          child: Container(
            ...
            child: urls.isNotEmpty
                ? CachedNetworkImage(
                    imageUrl:
                        posts[i]['thumbnail_url'] ?? urls.first,
                    fit: BoxFit.cover)
    ...
    return ListView(
      ...
      children: [
        _StatBlock('Tournament Record', [
          ...
          _StatLine('Win Rate', '${wr.toStringAsFixed(1)}%',
              GacomColors.success),
          _StatLine('Competitions Entered',
              '${profile['competitions_entered'] ?? 0}',
              GacomColors.textSecondary),
        ]),
        const SizedBox(height: 16),
        _StatBlock('Engagement', [
          _StatLine('Total Posts', '${profile['posts_count'] ?? 0}',
              GacomColors.textSecondary),
          _StatLine('Total Followers',
              _fmtN(profile['followers_count'] ?? 0), GacomColors.info),
        ]),
...
class _StatDiv extends StatelessWidget {
  @override
  Widget build(BuildContext context) =>
      Container(width: 0.7, height: 30, color: GacomColors.border);
}
