...
class _InstitutionPortalState extends State<InstitutionPortalScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _load() async {
    try {
      ...
      if (uid == null) return;
      // Find institution by login_email = current user email
      final email = SupabaseService.client.auth.currentUser?.email;
      final inst = await SupabaseService.client.from('institutions').select('*').eq('login_email', email ?? '').maybeSingle();
      if (inst == null) { setState(() => _loading = false); return; }

      final students = await SupabaseService.client.from('student_institutions')
          .select('*, student:profiles!student_id(display_name,username,avatar_url), progress:edu_progress(subject,xp,accuracy,streak)')
          .eq('institution_id', inst['id'] as String);

      final curricula = await SupabaseService.client.from('institution_curricula')
          .select('*').eq('institution_id', inst['id'] as String).order('created_at', ascending: false);

      if (mounted) setState(() {
        _institution = inst;
        _students = List<Map<String,dynamic>>.from(students as List);
        _curricula = List<Map<String,dynamic>>.from(curricula as List);
        _loading = false;
      });
