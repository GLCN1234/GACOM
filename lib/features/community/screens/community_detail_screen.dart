...
class _CommunityDetailScreenState extends ConsumerState<CommunityDetailScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _load() async {
    ...
    try {
      final comm = await SupabaseService.client.from('communities').select('*, created_by_profile:profiles!created_by(username, display_name, avatar_url)').eq('id', widget.communityId).single();
      final subs = await SupabaseService.client.from('communities').select('*').eq('parent_id', widget.communityId).order('members_count', ascending: false).limit(10);
      final ps = await SupabaseService.client.from('posts').select('*, author:profiles!author_id(username, display_name, avatar_url, verification_status)').eq('community_id', widget.communityId).eq('is_deleted', false).order('created_at', ascending: false).limit(20);
      bool isMember = false;
      if (userId != null) {
        final check = await SupabaseService.client.from('community_members').select('id').eq('community_id', widget.communityId).eq('user_id', userId).maybeSingle();
        isMember = check != null;
      }
      if (mounted) setState(() { _community = comm; _subCommunities = List<Map<String, dynamic>>.from(subs); _posts = List<Map<String, dynamic>>.from(ps); _isMember = isMember; _loading = false; });
    } catch (e) {
  ...
  Future<void> _join() async {
    ...
    try {
      await SupabaseService.client.from('community_members').insert({'community_id': widget.communityId, 'user_id': userId});
      if (mounted) { setState(() { _isMember = true; _joining = false; }); GacomSnackbar.show(context, 'Joined ${_community?['name']}!', isSuccess: true); }
    } catch (e) {
      if (mounted) { setState(() => _joining = false); GacomSnackbar.show(context, 'Failed to join', isError: true); }
    }
  }
      ...
      body: NestedScrollView(
            ...
            flexibleSpace: FlexibleSpaceBar(
              background: Stack(
                ...
                children: [
                  ...
                  Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.transparent, GacomColors.obsidian.withOpacity(0.8)], begin: Alignment.topCenter, end: Alignment.bottomCenter))),
                ],
              ),
            ),
          ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      ClipRRect(
                        ...
                        child: c['icon_url'] != null
                            ...
                            : Container(width: 64, height: 64, decoration: const BoxDecoration(gradient: GacomColors.orangeGradient), child: Center(child: Text((c['name'] ?? 'G')[0], style: const TextStyle(fontFamily: 'Rajdhani', fontSize: 32, fontWeight: FontWeight.w700, color: Colors.white)))),
                      ),
                  ...
                  if (c['description'] != null) Text(c['description'], style: const TextStyle(color: GacomColors.textSecondary, height: 1.5)),
                  ...
                  Row(
                    children: [
                      ...
                      if (_isMember) ...[
                        ...
                        GacomButton(label: '', width: 44, height: 44, icon: const Icon(Icons.chat_bubble_outline_rounded, color: Colors.white, size: 18), onPressed: () => context.go('/chat')),
                      ],
                    ],
                  ),
                  const SizedBox(height: 16),
        ...
        body: TabBarView(
          ...
          children: [
            ...
            _SubCommunitiesTab(communityId: widget.communityId, subs: _subCommunities, isMember: _isMember),
...
class _PostsTab extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (posts.isEmpty) return const Center(child: Text('No posts yet. Be the first!', style: TextStyle(color: GacomColors.textMuted)));
      ...
      itemBuilder: (_, i) {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                CircleAvatar(radius: 18, backgroundColor: GacomColors.border, backgroundImage: author['avatar_url'] != null ? CachedNetworkImageProvider(author['avatar_url']) : null, child: author['avatar_url'] == null ? Text((author['display_name'] ?? 'G')[0], style: const TextStyle(color: GacomColors.textPrimary)) : null),
                const SizedBox(width: 10),
...
class _SubCommunitiesTabState extends State<_SubCommunitiesTab> {
  ...
  @override
  void initState() { super.initState(); _subs = widget.subs; }
  ...
  void _showCreateDialog() {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('Create Sub-Community', style: TextStyle(fontFamily: 'Rajdhani', fontWeight: FontWeight.w700, color: GacomColors.textPrimary)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            GacomTextField(controller: nameCtrl, label: 'Name', hint: 'Sub-community name'),
            const SizedBox(height: 12),
            GacomTextField(controller: descCtrl, label: 'Description', hint: 'What is this sub-community about?', maxLines: 3),
          ],
        ),
        actions: [
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        if (widget.isMember)
          GacomButton(label: '+ CREATE SUB-COMMUNITY', isOutlined: true, height: 48, onPressed: _showCreateDialog),
        const SizedBox(height: 16),
        if (_subs.isEmpty) const Center(child: Text('No sub-communities yet.', style: TextStyle(color: GacomColors.textMuted))),
...
class _CompetitionsTabState extends ConsumerState<_CompetitionsTab> {
  ...
  Future<void> _load() async {
    final data = await SupabaseService.client.from('competitions').select('*').eq('community_id', widget.communityId).order('starts_at').limit(10);
