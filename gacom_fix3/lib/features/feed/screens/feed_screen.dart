    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Media — handles both images and videos
          if (mediaUrls.isNotEmpty)
            postType == 'video'
                ? _VideoPostPlayer(url: mediaUrls.first as String)
                : ClipRRect(
                    child: CachedNetworkImage(
                      ...
                      fit: BoxFit.cover,
                      placeholder: (_, __) =>
                          Container(height: 280, color: GacomColors.cardDark),
...
// ── Video post player ─────────────────────────────────────────────────────────
// Inline video player for feed posts. Tap to play/pause. Auto-disposes.
    ...
    if (_error) {
      return Container(
        ...
        child: const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          SizedBox(height: 8),
          Text('Video unavailable', style: TextStyle(color: GacomColors.textMuted, fontSize: 13)),
        ])),
      );
    }
          ...
          children: [
              ...
              child: Container(
                ...
                child: ValueListenableBuilder<VideoPlayerValue>(
                  ...
                  builder: (_, v, __) {
                    ...
                    return Text(
                      '${_fmt(pos)} / ${_fmt(dur)}',
                      style: const TextStyle(color: Colors.white, fontSize: 10, fontFamily: 'Rajdhani'),
                    );
                  },
                ),
              ),
            ...
            Positioned(
              ...
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                decoration: BoxDecoration(
                  color: GacomColors.deepOrange.withOpacity(0.85),
                  borderRadius: BorderRadius.circular(6),
                ),
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.videocam_rounded, color: Colors.white, size: 12),
                  SizedBox(width: 4),
                  Text('VIDEO', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w700, fontFamily: 'Rajdhani', letterSpacing: 0.8)),
                ]),
              ),
            ),
          ],
