...
class _MainShellState extends ConsumerState<MainShell> with SingleTickerProviderStateMixin {
  ...
  // Home | Blog | [+] | Chat | Community | Profile
  final _tabs = const [
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ FIX: only real admins/moderators see the admin bar — exco does NOT
    ...
    return Scaffold(
      ...
      bottomNavigationBar: _buildNav(isPrivileged, isExco, role),
    );
  }

  Widget _buildNav(bool isPrivileged, bool isExco, String role) {
    return ClipRRect(
        ...
        child: Container(
          ...
          child: SafeArea(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              // ✅ Admin bar — only for admin/super_admin/moderator
              if (isPrivileged) _AdminBar(role: role),
              // ✅ Exco bar — separate, goes to exco dashboard not admin
...
class _CompetitionFab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Stack(children: [
      Container(
        width: 48, height: 48,
        decoration: BoxDecoration(color: GacomColors.cardDark, shape: BoxShape.circle, border: Border.all(color: GacomColors.deepOrange.withOpacity(0.5), width: 1.5), boxShadow: [BoxShadow(color: GacomColors.deepOrange.withOpacity(0.25), blurRadius: 12)]),
        child: const Icon(Icons.sports_esports_rounded, color: GacomColors.deepOrange, size: 22),
      ),
      if (badge > 0) Positioned(right: 0, top: 0, child: Container(width: 18, height: 18, decoration: const BoxDecoration(color: GacomColors.error, shape: BoxShape.circle), child: Center(child: Text(badge > 9 ? '9+' : '$badge', style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w800))))),
    ]),
  );
}
...
// ── Exco bar — role=exco only, routes to /exco-dashboard NOT /admin ───────────
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(Icons.badge_rounded, color: GacomColors.info, size: 14),
        const SizedBox(width: 6),
        Text('MY TEAM DASHBOARD', style: TextStyle(fontFamily: 'Rajdhani', fontWeight: FontWeight.w800, fontSize: 11, color: GacomColors.info, letterSpacing: 1.2)),
        const SizedBox(width: 6),
        Icon(Icons.arrow_forward_ios_rounded, color: GacomColors.info, size: 10),
      ]),
    ),
  );
}
