  ...
  Future<void> _publish() async {
    ...
    try {
      ...
      for (final file in _selectedMedia) {
        ...
        // ✅ FIXED: path must start with userId folder — matches storage RLS policy
      ...
      if (mounted) {
        ...
        GacomSnackbar.show(
          context,
          'Failed to publish. ${e.toString().contains('storage') ? 'Storage not configured — ask admin to set up post-media bucket.' : 'Check your connection.'}',
          isError: true,
        );
      }
    }
    ...
    return Scaffold(
        ...
        actions: [
          TextButton(
            ...
            child: _loading
                ...
                : const Text('POST',
                    style: TextStyle(color: GacomColors.deepOrange, fontFamily: 'Rajdhani', fontWeight: FontWeight.w700, fontSize: 16)),
          ),
      ...
      body: ListView(
        ...
        children: [
          // Current user info
          FutureBuilder(
            future: SupabaseService.client.from('profiles')
                .select('display_name, avatar_url')
                .eq('id', SupabaseService.currentUserId ?? '')
                .maybeSingle(),
            builder: (ctx, snap) {
              final name = (snap.data as Map?)?['display_name'] ?? 'You';
              ...
              return Row(children: [
                CircleAvatar(
                  ...
                  child: avatar == null
                      ? Text(name[0].toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700))
                      : null,
                ),
                    ...
                    ClipRRect(
                      ...
                      child: FutureBuilder<Uint8List>(
                        ...
                        builder: (_, snap) {
                          ...
                          return Container(width: 100, height: 100, color: GacomColors.cardDark);
                        },
                      ),
                    ),
                    Positioned(
          ...
          Row(children: [
            ...
            TextButton(onPressed: _addTag, child: const Text('ADD', style: TextStyle(color: GacomColors.deepOrange))),
          ]),
          ...
          // Media action row
          Row(children: [
            _MediaButton(icon: Icons.image_rounded, label: 'Photo', onTap: () => _pickMedia()),
            const SizedBox(width: 12),
            _MediaButton(icon: Icons.videocam_rounded, label: 'Video', onTap: () => _pickMedia(video: true)),
          ]),
