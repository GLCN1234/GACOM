  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        // ✅ FIX: safe URL cast — media_urls comes from Postgres as List<dynamic>
        // Direct cast to String can throw; use toString() as safe fallback.
        if (mediaUrls.isNotEmpty) ...[
          Builder(builder: (_) {
            final rawUrl = mediaUrls.first;
            final url = rawUrl is String ? rawUrl : rawUrl?.toString() ?? '';
            if (url.isEmpty) return const SizedBox.shrink();
            final isVideo = postType == 'video' || postType == 'clip' || url.contains('.mp4') || url.contains('.mov');
            if (isVideo) {
              // Video placeholder — show play icon with thumbnail if available
              return Container(
                width: double.infinity, height: 220,
                color: GacomColors.cardDark,
                child: const Center(child: Icon(Icons.play_circle_fill_rounded, color: GacomColors.deepOrange, size: 64)),
              );
            }
            return ClipRRect(
              child: CachedNetworkImage(
                ...
                height: 280,
                fit: BoxFit.cover,
                placeholder: (_, __) => Container(height: 280, color: GacomColors.cardDark,
                  child: const Center(child: CircularProgressIndicator(color: GacomColors.deepOrange, strokeWidth: 2))),
                errorWidget: (_, __, ___) => Container(height: 180, color: GacomColors.cardDark,
                  child: const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(Icons.image_not_supported_outlined, color: GacomColors.textMuted, size: 40),
                    SizedBox(height: 8),
