...
class _ChatListScreenState extends ConsumerState<ChatListScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      // Step 1: get chat IDs this user is in
      ...
      // ✅ FIX: ONE batch fetch of all members for all chats — no recursive per-chat queries
      // This avoids the N+1 loop that was hitting the RLS recursion bug.
      final allMembers = await SupabaseService.client
          .from('chat_members')
          .select('chat_id, user_id')
          .inFilter('chat_id', chatIds);

      // Build map: chatId → [other user IDs]
      final chatOtherIds = <String, List<String>>{};
      for (final m in allMembers as List) {
        final cid = m['chat_id'] as String;
        final uid = m['user_id'] as String;
        if (uid != userId) chatOtherIds.putIfAbsent(cid, () => []).add(uid);
      }

      // ✅ ONE batch profiles fetch for all other users
      final allOtherIds = chatOtherIds.values.expand((ids) => ids).toSet().toList();
      ...
      for (final chat in chats as List) {
        ...
        final otherIds = chatOtherIds[chatId] ?? [];
        Map<String, dynamic>? otherUser;
        if (chat['type'] == 'direct' && otherIds.isNotEmpty) {
          otherUser = profilesById[otherIds.first];
        }
        enriched.add({...chat as Map<String, dynamic>, '_other_user': otherUser});
      }

      // Active users (story row)
      final following = await SupabaseService.client
          .from('follows').select('following_id').eq('follower_id', userId).limit(20);
  ...
  Future<void> _startDm(String otherUserId) async {
    ...
    try {
      ...
      try {
        ...
        // RPC not yet deployed — fallback to checking enriched chats in memory
        for (final chat in _chats) {
          if (chat['type'] == 'direct') {
            final other = chat['_other_user'] as Map?;
            if (other?['id'] == otherUserId) {
  ...
  void _showNewChatSheet() {
    showModalBottomSheet(
      ...
      builder: (_) => _NewChatSheet(onNewMessage: (uid) { Navigator.pop(context); _startDm(uid); }),
    );
  }
      ...
      body: SafeArea(child: Column(children: [
        ...
        Padding(padding: const EdgeInsets.fromLTRB(20, 16, 8, 0), child: Row(children: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => [_menuItem('new_chat', Icons.chat_outlined, 'New Chat'), _menuItem('new_community', Icons.group_add_outlined, 'New Community')],
          ),
        ])),
        ...
        SizedBox(height: 90, child: ListView.builder(
          ...
          itemBuilder: (_, i) {
            ...
            return GestureDetector(onTap: () => _startDm(u['id']), child: Container(width: 60, margin: const EdgeInsets.only(right: 14),
              child: Column(children: [
                Stack(children: [
                  Container(width: 54, height: 54, padding: const EdgeInsets.all(2.5),
                    decoration: BoxDecoration(shape: BoxShape.circle, gradient: isOnline ? GacomColors.orangeGradient : null, color: isOnline ? null : GacomColors.border),
                    child: CircleAvatar(backgroundColor: GacomColors.cardDark, backgroundImage: u['avatar_url'] != null ? CachedNetworkImageProvider(u['avatar_url']) : null,
                      child: u['avatar_url'] == null ? Text(name.isNotEmpty ? name[0] : '?', style: const TextStyle(color: GacomColors.textPrimary, fontWeight: FontWeight.bold, fontSize: 18)) : null)),
                  if (isOnline) Positioned(right: 1, bottom: 1, child: Container(width: 13, height: 13,
  ...
  PopupMenuItem<String> _menuItem(String value, IconData icon, String label) => PopupMenuItem(
    value: value,
    child: Row(children: [Icon(icon, color: GacomColors.textSecondary, size: 18), const SizedBox(width: 10), Text(label, style: const TextStyle(color: GacomColors.textPrimary, fontFamily: 'Rajdhani', fontWeight: FontWeight.w600, fontSize: 14))]),
  );
}
    ...
    return Material(color: Colors.transparent, child: InkWell(onTap: onTap, splashColor: GacomColors.deepOrange.withOpacity(0.04),
      child: Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10), child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(preview ?? 'Start a conversation', style: TextStyle(color: preview != null ? GacomColors.textSecondary : GacomColors.textMuted, fontSize: 13, height: 1.3), maxLines: 1, overflow: TextOverflow.ellipsis),
        ])),
        const SizedBox(width: 10),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text(timeago.format(lastAt, allowFromNow: true), style: TextStyle(color: unread > 0 ? GacomColors.deepOrange : GacomColors.textMuted, fontSize: 11, fontFamily: 'Rajdhani', fontWeight: unread > 0 ? FontWeight.w700 : FontWeight.w500)),
          const SizedBox(height: 4),
          if (unread > 0)
            Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3), decoration: BoxDecoration(color: GacomColors.deepOrange, borderRadius: BorderRadius.circular(50)), child: Text('$unread', style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700)))
          else
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    Container(width: 96, height: 96, decoration: BoxDecoration(color: GacomColors.deepOrange.withOpacity(0.08), shape: BoxShape.circle, border: Border.all(color: GacomColors.deepOrange.withOpacity(0.2), width: 1.5)), child: const Icon(Icons.chat_bubble_outline_rounded, size: 44, color: GacomColors.deepOrange)).animate().scale(curve: Curves.elasticOut),
...
class _NewChatSheetState extends State<_NewChatSheet> {
  ...
  Future<void> _search(String q) async {
    if (q.trim().isEmpty) { setState(() => _users = []); return; }
  ...
  @override
  Widget build(BuildContext context) {
    return ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(28)),
      child: BackdropFilter(filter: ImageFilter.blur(sigmaX: 20, sigmaY: 20),
        child: Container(
          ...
          child: DraggableScrollableSheet(expand: false, initialChildSize: 0.75, maxChildSize: 0.95,
            builder: (_, scroll) => Column(children: [
              ...
              Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: TextField(controller: _ctrl, autofocus: true, onChanged: _search, style: const TextStyle(color: GacomColors.textPrimary, fontSize: 15),
                decoration: InputDecoration(hintText: 'Search by name or @username…', hintStyle: const TextStyle(color: GacomColors.textMuted), prefixIcon: const Icon(Icons.search_rounded, color: GacomColors.textMuted), filled: true, fillColor: GacomColors.surfaceDark,
              ...
              Expanded(child: _searching
                ? const Center(child: CircularProgressIndicator(color: GacomColors.deepOrange))
                : _users.isEmpty
                  ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.person_search_rounded, size: 48, color: GacomColors.border), const SizedBox(height: 12), Text(_ctrl.text.isEmpty ? 'Search for gamers' : 'No users found', style: const TextStyle(color: GacomColors.textMuted))]))
                  : ListView.builder(controller: scroll, itemCount: _users.length, itemBuilder: (_, i) {
                      ...
                      return ListTile(
                        ...
                        trailing: isOnline ? Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: GacomColors.success.withOpacity(0.15), borderRadius: BorderRadius.circular(50)), child: const Text('Online', style: TextStyle(color: GacomColors.success, fontSize: 11, fontFamily: 'Rajdhani', fontWeight: FontWeight.w600))) : null,
                        onTap: () => widget.onNewMessage(u['id'] as String),
