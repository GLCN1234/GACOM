...
class _StoreScreenState extends ConsumerState<StoreScreen>
  ...
  Future<void> _checkAdmin() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() =>
            _isAdmin = (results[0] as bool) || (results[1] as bool));
      }
    } catch (_) {
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await SupabaseService.client
          .from('products')
          .select(
              'id, name, description, price, condition, brand, images, tags, '
  ...
  void _showProductSheet(Map<String, dynamic> product) {
    ...
    final sellerName = product['seller_name'] as String? ??
        ((product['seller'] as Map?)?['display_name'] as String?) ??
        'GACOM Store';
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (_, scroll) => ListView(
          ...
          children: [
              ...
              child: AspectRatio(
                ...
                child: images.isNotEmpty
                    ? CachedNetworkImage(
                        ...
                        fit: BoxFit.cover,
                        // ✅ FIX: proper placeholder and error widget
                        ...
                        errorWidget: (_, __, ___) => _CategoryFallbackImage(
                          category: product['category'] as String? ?? '',
                          name: product['name'] as String? ?? '',
                        ),
                      )
                    : _CategoryFallbackImage(
                        category: product['category'] as String? ?? '',
                        name: product['name'] as String? ?? '',
                      ),
              ),
            ...
            if (isDemo)
              GacomButton(
                label: 'COMING SOON — Demo Item',
                onPressed: () {
                  Navigator.pop(ctx);
                  GacomSnackbar.show(context, 'This is a demo listing!');
                },
              )
            else
              // ✅ FIX: Navigate to product detail screen (order form + Paystack)
              //    instead of the wrong "message seller" shortcut.
              GacomButton(
                label: 'ORDER NOW',
                onPressed: () {
                  Navigator.pop(ctx);
          ...
          builder: (_, scroll) => Padding(
            ...
            child: ListView(controller: scroll, children: [
              ...
              GestureDetector(
                onTap: () async {
                  ...
                  try {
                    ...
                    final path =
                        'products/${DateTime.now().millisecondsSinceEpoch}.$ext';
                    final mimeType = ext.toLowerCase() == 'png'
                        ? 'image/png'
                        : ext.toLowerCase() == 'webp'
                            ? 'image/webp'
                            : 'image/jpeg';
                    final url = await SupabaseService.uploadFile(
                        bucket: AppConstants.productImageBucket,
                        path: path,
                        bytes: bytes,
                    ...
                    GacomSnackbar.show(ctx, 'Image upload failed',
                        isError: true);
                  }
                },
                child: Container(
                          ...
                          : const Column(
                              ...
                              children: [
                                Icon(Icons.add_photo_alternate_outlined,
                                    color: GacomColors.deepOrange, size: 32),
                                SizedBox(height: 8),
              ...
              GacomTextField(
                  controller: nameCtrl,
                  label: 'Product Name *',
                  hint: 'e.g. Gaming Headset Pro',
                  prefixIcon: Icons.inventory_2_rounded),
              const SizedBox(height: 12),
              GacomTextField(
                  controller: brandCtrl,
                  label: 'Brand',
                  hint: 'e.g. Sony, Corsair',
              ...
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: ['accessories', 'apparel', 'games', 'collectibles']
                ...
                child: GacomButton(
                  ...
                  onPressed: () async {
                    if (nameCtrl.text.trim().isEmpty ||
                      ...
                      GacomSnackbar.show(
                          ctx, 'Name, description and price required',
                          isError: true);
                      return;
                    }
                    try {
                      ...
                      if (ctx.mounted) {
                        GacomSnackbar.show(ctx, 'Failed: ${e.toString()}',
                            isError: true);
                      }
                    }
                  },
    ...
    return Scaffold(
        ...
        actions: [
          if (_isAdmin) ...[
            IconButton(
              ...
              onPressed: () => _showDeliveryZonesSheet(context),
            ),
            IconButton(
      ...
      body: TabBarView(
        ...
        children: [
          ...
          _MyListingsTab(
            isAdmin: _isAdmin,
            onAddProduct: () => _showAddProductSheet(context),
          ),
        ],
      ),
    );
...
class _MyListingsTab extends ConsumerStatefulWidget {
  ...
  const _MyListingsTab(
      {required this.isAdmin, required this.onAddProduct});
  @override
...
class _MyListingsTabState extends ConsumerState<_MyListingsTab> {
          ...
          child: Column(
            ...
            const Text('No listings yet',
                style: TextStyle(
                    ...
                    fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            const Text('Add products you want to sell',
      ...
      itemBuilder: (_, i) {
        ...
        return Container(
          ...
          child: Column(
              ...
              children: [
                Row(children: [
                  // ✅ FIX: proper image with error fallback in listings too
                  ClipRRect(
                    borderRadius: BorderRadius.circular(10),
                    child: SizedBox(
                      width: 56,
                      ...
                      child: Column(
                        ...
                        Text(p['name'] ?? '',
                            style: const TextStyle(
                                ...
                                fontSize: 14)),
                        Text(
                            '₦${(p['price'] as num?)?.toStringAsFixed(0) ?? '0'} · ${p['condition'] ?? 'new'}',
                            style: const TextStyle(
                                color: GacomColors.deepOrange,
                                fontSize: 12,
                                fontFamily: 'Rajdhani',
                  ...
                  _CondBadge((p['is_active'] as bool? ?? true)
                      ? 'active'
                      : 'inactive'),
                ]),
                const SizedBox(height: 10),
                Wrap(spacing: 8, runSpacing: 8, children: [
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                          color:
                              GacomColors.info.withOpacity(0.08),
                          borderRadius: BorderRadius.circular(8),
                          border: Border.all(
                              color:
                                  GacomColors.info.withOpacity(0.4))),
                      child: const Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(Icons.edit_rounded,
                                size: 14, color: GacomColors.info),
                            SizedBox(width: 6),
                            Text('Edit',
                                style: TextStyle(
                                    color: GacomColors.info,
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        gradient: LinearGradient(colors: [
                          GacomColors.error.withOpacity(0.9),
                          GacomColors.error
                        ]),
                        borderRadius: BorderRadius.circular(8),
                      ),
  ...
  void _showEditSheet(BuildContext context, Map<String, dynamic> p) {
    ...
    final stockCtrl =
        TextEditingController(text: '${p['stock'] ?? 1}');
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: GacomColors.cardDark,
            ...
            Row(children: [
              Container(
                ...
                child: const Icon(Icons.edit_rounded,
                    color: GacomColors.info, size: 20),
              ),
              const SizedBox(width: 12),
            ...
            GacomButton(
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  GacomSnackbar.show(ctx, 'Failed: $e',
                      isError: true);
                }
              },
            ),
...
// ── Category-aware fallback image shown when there's no URL ──────────────────
class _CategoryFallbackImage extends StatelessWidget {
  final String category;
  final String name;
  const _CategoryFallbackImage(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(_icon, color: GacomColors.deepOrange, size: 40),
...
class _CondBadge extends StatelessWidget {
  ...
  Color get _color {
    if (condition == 'new' || condition == 'active') {
      return GacomColors.success;
    }
    if (condition == 'used' || condition == 'inactive') {
...
// ─────────────────────────────────────────────────────────────────────────────
// DELIVERY ZONES MANAGEMENT (unchanged, kept intact)
// ─────────────────────────────────────────────────────────────────────────────
...
class _DeliveryZonesSheet extends ConsumerStatefulWidget {
  @override
  ConsumerState<_DeliveryZonesSheet> createState() =>
      _DeliveryZonesSheetState();
}

class _DeliveryZonesSheetState
    extends ConsumerState<_DeliveryZonesSheet> {
  ...
  void _showAddZoneDialog() {
    final existing =
        _zones.map((z) => z['state_name'] as String).toSet();
    final available =
        _kNigerianStates.where((s) => !existing.contains(s)).toList();
        ...
        builder: (ctx, setDlg) => AlertDialog(
          ...
          actions: [
            TextButton(
                ...
                child: const Text('Cancel',
                    style:
                        TextStyle(color: GacomColors.textMuted))),
            ElevatedButton(
              style: ElevatedButton.styleFrom(
                  backgroundColor: GacomColors.deepOrange,
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(10))),
              onPressed: available.isEmpty || selectedState == null
                  ? null
                  : () async {
                      final fee =
  ...
  Future<void> _bulkSeedAllStates() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        backgroundColor: GacomColors.cardDark,
        title: const Text('Seed All States?',
            style: TextStyle(
                fontFamily: 'Rajdhani',
                fontWeight: FontWeight.w800,
                color: GacomColors.textPrimary)),
        content: const Text(
          ...
          'States already added will be skipped.',
          style: TextStyle(
              color: GacomColors.textSecondary,
              height: 1.5,
              fontSize: 13)),
    ...
    final existing =
        _zones.map((z) => z['state_name'] as String).toSet();
    final toInsert = _kNigerianStates
        .where((s) => !existing.contains(s))
        .toList();
    if (toInsert.isEmpty) {
      GacomSnackbar.show(context, 'All states already added!',
          isSuccess: true);
    ...
    try {
      for (int i = 0; i < toInsert.length; i += 10) {
        final batch = toInsert.sublist(
            i, i + 10 > toInsert.length ? toInsert.length : i + 10);
        ...
        Padding(
          ...
          child: Column(children: [
            ...
            Row(children: [
              Container(
                  ...
                  child: const Icon(Icons.local_shipping_rounded,
                      color: GacomColors.info, size: 22)),
              const SizedBox(width: 12),
              const Expanded(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                    ...
                    Text('Delivery Zones',
                        style: TextStyle(
                            ...
                            fontWeight: FontWeight.w800,
                            color: GacomColors.textPrimary)),
                    Text('Manually set fee per state',
                        style: TextStyle(
                            color: GacomColors.textMuted,
                            fontSize: 12)),
                  ])),
            ...
            if (_zones.isNotEmpty)
              Row(children: [
                _ZoneChip('${_zones.length} / 37 States',
                    Icons.map_rounded, GacomColors.info),
                const SizedBox(width: 8),
                _ZoneChip(
                  ...
                  ? Center(
                      ...
                      child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            const Text(
                              'Add zones manually or tap "Seed All" to populate all 37 Nigerian states.',
                              textAlign: TextAlign.center,
                              style: TextStyle(
                                  color: GacomColors.textMuted,
                                  fontSize: 13,
...
class _ZoneTileState extends State<_ZoneTile> {
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: _editing
            ? GacomColors.deepOrange.withOpacity(0.04)
            : GacomColors.cardDark,
        borderRadius: BorderRadius.circular(14),
        border: Border.all(
            color: _editing
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color:
                                GacomColors.error.withOpacity(0.3))),
                    child: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: const [
                          Icon(Icons.delete_outline_rounded,
                              size: 16, color: GacomColors.error),
                          SizedBox(width: 6),
                          Text('Remove',
                              style: TextStyle(
                                  color: GacomColors.error,
          ...
          : GestureDetector(
              ...
              child: Row(children: [
                ...
                const Icon(Icons.edit_rounded,
                    color: GacomColors.textMuted, size: 15),
              ]),
            ),
    );
  }
